...
class Methods {
    ...
    loadinfoWindow(obj) {
        ...
        var tableStr = '';
        if (obj.Values) {
            tableStr += '<table cellspacing="0" cellpadding="0" border="0" class="table1-popupTableWaterExit_p"><thead>';
            for (let i = 0; i < obj.Values.length; i++) {
                const element = obj.Values[i];
                tableStr += '<tr><td class="table-td-l-popupTableWaterExit_p">' + element.Label + '</td><td class="table-td-r-popupTableWaterExit_p">' + element.Value + '</td></tr>';
            }
            tableStr += '</thead></table>';
        }else{
            tableStr = '<div class="table-td-l-popupTableWaterExit_p" title="'+obj.Name+'">'+obj.Name+'</div>';
        }
        //动态添加气泡窗口DIV  
        var infoDiv = '<div id="popup-TableWaterExit_p' + popupID + '" class="popupTableWaterExit_p">' +
            '<div id="' + popupID + '" class="content-popupTableWaterExit_p">' + 
            '<div id="table' + popupID + '" class="table-popupTableWaterExit_p-'+obj.Type+'">' + tableStr + '</div>' + 
            '</div>' +
            '<div class="tip-container-popupTableWaterExit_p">' +
            '<div class="tip-b-popupTableWaterExit_p-'+obj.Type+'"></div><div class="tip-popupTableWaterExit_p-'+obj.Type+'"></div>' +
            '</div>' +
            '</div>';
    ...
    positionPopUp(obj) {//刷新气泡窗口位置
        const popupID = this.popupName + obj.id;
        var tempPopup = $('#popup-TableWaterExit_p' + popupID);
