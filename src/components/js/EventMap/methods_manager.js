...
var esriApi, mapView, webTileMap1Temp, dynamicMapServiceLayerTemp, seattleLayer, drwdLayer, drwdPointLayer, exitPointLayer, isbool = false, isLoadMap = false, isActive = false, listData = [], exitPoints = [], lableData = [], itemInfo, funGetAreaCoordinatesInfo;
class Methods {
  ...
  initMap(obj) { // 初始化地图,并设置中心点等
    ...
    view.when(function (e) {
      ...
      popupTableWater.initPopupEvent(view, mapView.spatialReference, "jcdPopup");
      popupTableWaterExit.initPopupEvent(view, mapView.spatialReference, "exitPopup");
      isLoadMap = true;
      // self.drwdListPoints();
    ...
    funGetAreaCoordinatesInfo = function GetAreaCoordinatesInfo() {
      ...
      if (itemInfo && itemInfo.Geometry) {
        ...
        if (result.GeoType == "Point") {
          if(itemInfo.EventType == "pipeburst"){
            listData.push({ Level: itemInfo.Level, X: result.Geo.X, Y: result.Geo.Y });
            self.drwdListPoints();
          }else{
            diffuseBubbles.destroyByPopupName("eventMap");
            var _up = "P";
            if (itemInfo.EventType == "flow" || itemInfo.EventType == "minflowatnight") {
              _up = "F";
            }
            diffuseBubbles.loadPopup([{ id: 0, type: "red", up: _up, x: result.Geo.X, y: result.Geo.Y }]);
          }
    ...
    function setupHoverTooltip(layerview) {
      ...
      var hitTest = esriApi.promiseUtils.debounce(function (event) {
        return view.hitTest(event).then(function (hit) {
          ...
          // });
          var results = hit.results.filter(function (result) {
            return (result.graphic.layer === drwdLayer || result.graphic.layer === exitPointLayer);
          });
  ...
  drwdLablePoints() {
    if (isLoadMap && lableData.length > 0) {
      ...
      for (let i = 0; i < lableData.length; i++) {
        ...
        var gra = new esriApi.Graphic({ geometry: point, symbol: symbol, attributes: obj });
        drwdLayer.add(gra);
      }
      if (isActive && tempData.length > 0) {
        popupTableWater.destroy();
        popupTableWater.loadPopup(tempData);
      }
    }
    ...
    if (isLoadMap && exitPoints.length > 0) {
      ...
      for (let i = 0; i < exitPoints.length; i++) {
        ...
        if (obj.X && obj.Y) {
          ...
          var gra = new esriApi.Graphic({ geometry: pt, symbol: symbol, attributes: obj });
  ...
  drwdListPoints() {
    if (isLoadMap && listData.length > 0) {
      ...
      // if (tempData.length > 0) {
      //   diffuseBubbles.destroyByPopupName("eventMap");
      //   diffuseBubbles.loadPopup(tempData);
      // }
    }
  }
  ...
  LocationMapXY(X, Y){
    var pt = new esriApi.Point({ x: X, y: Y, spatialReference: mapView.spatialReference });
    mapView.center = pt;
  }
}
