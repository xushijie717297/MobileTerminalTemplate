...
class Methods {
    ...
    loadinfoWindow(obj) {
        ...
        var tableStr = '';
        if (obj.Values) {
            tableStr += '<div class="table-td-l-popupAreaInformation" title="'+obj.Name+'">'+obj.Name+'</div>';
            tableStr += '<table cellspacing="0" cellpadding="0" border="0" class="table1-popupAreaInformation"><thead>';
            for (let i = 0; i < obj.Values.length; i++) {
                const element = obj.Values[i];
                tableStr += '<tr><td class="table-td-l-popupAreaInformation">' + element.Label + '</td><td class="table-td-r-popupAreaInformation"><div class="table-td-r-s-popupAreaInformation-'+obj.Type+'">' + element.Value + '</div></td></tr>';
            }
        ...
        var infoDiv = '<div id="popup-AreaInformation' + popupID + '" class="popupAreaInformation">' +
            '<div id="' + popupID + '" class="content-popupAreaInformation">' + 
            '<div id="table' + popupID + '" class="table-popupAreaInformation-'+obj.Type+'">' + tableStr + '</div>' + 
            '</div>' +
            '<div class="tip-container-popupAreaInformation">' +
            '<div class="tip-b-popupAreaInformation-'+obj.Type+'"></div><div class="tip-popupAreaInformation-'+obj.Type+'"></div>' +
            '</div>' +
            '</div>';
