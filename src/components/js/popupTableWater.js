...
class Methods {
    ...
    loadinfoWindow(obj) {
        ...
        var tableStr = '';
        if (obj.Values) {
            tableStr += '<table cellspacing="0" cellpadding="0" border="0" class="table1-popupTableWater"><thead>';
            for (let i = 0; i < obj.Values.length; i++) {
                const element = obj.Values[i];
                tableStr += '<tr><td class="table-td-l-popupTableWater">' + element.Label + '</td><td class="table-td-r-popupTableWater">' + element.Value + '</td></tr>';
            }
            tableStr += '</thead></table>';
        }else{
            tableStr = '<div class="table-td-l-popupTableWater" title="'+obj.Name+'">'+obj.Name+'</div>';
        }
        //动态添加气泡窗口DIV  
        var infoDiv = '<div id="popup-TableWater' + popupID + '" class="popupTableWater">' +
            '<div id="' + popupID + '" class="content-popupTableWater">' + 
            '<div id="table' + popupID + '" class="table-popupTableWater-'+obj.Type+'">' + tableStr + '</div>' + 
            '</div>' +
            '<div class="tip-container-popupTableWater">' +
            '<div class="tip-b-popupTableWater-'+obj.Type+'"></div><div class="tip-popupTableWater-'+obj.Type+'"></div>' +
            '</div>' +
            '</div>';
    ...
    positionPopUp(obj) {//刷新气泡窗口位置
        const popupID = this.popupName + obj.id;
        var tempPopup = $('#popup-TableWater' + popupID);
