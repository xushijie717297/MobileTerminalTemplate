<template>
<div class="box">
  <div :id="item.MonitoringPointName +1" class="wrap"></div>
...
export default {
  ...
  mounted() {
    ...
    let HistoryWindowOutList = this.item.HistoryWindowOutList;
    ...
    // this.$nextTick(()=>{
    //   console.log(this.iten,this.itemInfo)
  ...
  methods: {
    drawLine(chartData) {
      ...
      let itemName = this.item.MonitoringPointName + "1"
                ...
                [
                  {
                    name: "",
                    xAxis:
                      this.item.EventType == "pipeburst"
                        ? this.item.ContinueTime
                          ? this.$moment(this.item.BeginTime).format("HH:mm")
                          : ""
                        : this.$moment(this.item.BeginTime).format(
                            "YYYY-MM-DD HH:mm:ss"
                          ),
                  },
                  {
                    xAxis:
                      this.item.EventType == "pipeburst"
                        ? this.item.ContinueTime
                          ? this.$moment(this.item.EndTime).format("DD") -
                              this.$moment(this.item.BeginTime).format("DD") ==
                            0
                            ? this.$moment(this.item.EndTime).format("HH:mm")
                            : this.$moment(this.item.EndTime).format("23:45")
                          : ""
                        : this.$moment(this.item.EndTime).format(
                            "YYYY-MM-DD 12:00:00"
                          ),
                  },
                ],
